...
void PostOperatorCSV<solver_t>::PrintProbeE()
{
  ...
  for (const auto &idx : post_op->interp_op.GetProbes())
  {
    for (int i_dim = 0; i_dim < v_dim; i_dim++)
    {
      ...
      probe_E->table[format("E{}_{}_{}_re", idx, i_dim, m_ex_idx)] << val.real();
      if (HasComplexGridFunction<solver_t>())
      {
        probe_E->table[format("E{}_{}_{}_im", idx, i_dim, m_ex_idx)] << val.imag();
      }
    }
    i++;
  }
...
void PostOperatorCSV<solver_t>::PrintProbeB()
{
  ...
  for (const auto &idx : post_op->interp_op.GetProbes())
  {
    ...
    {
      auto val = probe_field[i * v_dim + i_dim];
      probe_B->table[format("B{}_{}_{}_re", idx, i_dim, m_ex_idx)] << val.real();
      if (HasComplexGridFunction<solver_t>())
      {
        probe_B->table[format("B{}_{}_{}_im", idx, i_dim, m_ex_idx)] << val.imag();
      }
    }
    i++;
  }
